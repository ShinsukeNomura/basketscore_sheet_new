  ...
  return (
    <div className="min-h-dvh bg-neutral-950 flex flex-col items-center justify-center px-6 gap-6">
        ...
        {polling ? (
          <>
            <h1 className="text-white font-black text-2xl mb-2">
              {locale === 'ja' ? 'プレミアムを有効化中...' : 'Activating Premium...'}
            </h1>
            <p className="text-white/40 text-sm leading-relaxed">
              {locale === 'ja' ? '決済を確認しています。\nそのままお待ちください。' : 'Verifying your payment.\nPlease wait a moment.'}
            </p>
          </>
        ) : confirmed ? (
          <>
            <h1 className="text-white font-black text-2xl mb-2">
              {locale === 'ja' ? 'プレミアム登録完了！' : 'Premium Activated!'}
            </h1>
            <p className="text-white/40 text-sm leading-relaxed">
              {locale === 'ja' ? 'ありがとうございます。\nすべての機能が使えるようになりました。' : 'Thank you!\nAll premium features are now unlocked.'}
            </p>
            <p className="text-white/20 text-xs mt-3">
              {locale === 'ja' ? '2秒後に試合作成画面へ移動します...' : 'Redirecting in 2 seconds...'}
            </p>
          </>
        ) : (
          <>
            <h1 className="text-white font-black text-xl mb-2">
              {locale === 'ja' ? '有効化に時間がかかっています' : 'Taking a bit longer than expected'}
            </h1>
            <p className="text-white/40 text-sm leading-relaxed">
              {locale === 'ja'
                ? '決済は完了しています。\n数分後にホームページで確認してください。'
      ...
      {!polling && (
        ...
        >
          {locale === 'ja' ? '試合を作成する' : 'Create a game'}
